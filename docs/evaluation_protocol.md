...
1. Run the Go test suite to verify correctness.
2. Execute internal Go benchmarks for eager and compiled workloads.
3. Export runtime benchmark results as JSON files.
4. Generate external baseline result files using PyTorch and ONNX Runtime.
5. Compare exported results using the repository's comparison tool.
6. Derive and validate the measured planner using CPU and CUDA result files.
...
1. CPU-only runtime execution.
2. Hybrid execution with the default threshold planner.
3. Hybrid execution with the measured planner.
4. Eager execution of supported operations.
5. Compiled graph execution using the repository's compiler layer.
6. Runtime comparison against PyTorch.
7. Runtime comparison against ONNX Runtime.
