...
func AddCUDA(a, b *Tensor) (*Tensor, error) {
	...
	if !cuda.Available() { return nil, fmt.Errorf("cuda unavailable") }
	ac, bc := a.Clone(), b.Clone()
	if err := ac.ToCUDA(); err != nil { return nil, err }
	defer ac.Free()
	if err := bc.ToCUDA(); err != nil { return nil, err }
	defer bc.Free()
	out, _ := New(a.Shape, DeviceCPU)
	outBuf, err := cuda.MallocFloat32(out.Numel())
	if err != nil { return nil, err }
	defer cuda.Free(outBuf)
	if err := cuda.LaunchAddFloat32(ac.cudaBuf.ptr, bc.cudaBuf.ptr, outBuf, out.Numel()); err != nil { return nil, err }
	if err := cuda.CopyFloat32DeviceToHost(outBuf, out.Data); err != nil { return nil, err }
	return out, nil
}
...
func ReLUCUDA(a *Tensor) (*Tensor, error) {
	...
	if err != nil { return nil, err }
	defer cuda.Free(outBuf)
	if err := cuda.LaunchReLUFloat32(ac.cudaBuf.ptr, outBuf, out.Numel()); err != nil { return nil, err }
	if err := cuda.CopyFloat32DeviceToHost(outBuf, out.Data); err != nil { return nil, err }
	return out, nil
}
...
func MatMulCUDA(a, b *Tensor) (*Tensor, error) {
	...
	if err != nil { return nil, err }
	defer cuda.Free(outBuf)
	if err := cuda.LaunchMatMulFloat32(ac.cudaBuf.ptr, bc.cudaBuf.ptr, outBuf, m, k1, n); err != nil { return nil, err }
	if err := cuda.CopyFloat32DeviceToHost(outBuf, out.Data); err != nil { return nil, err }
	return out, nil
}
