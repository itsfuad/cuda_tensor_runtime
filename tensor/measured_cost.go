...
func LoadMeasuredCostModel(cpuResultsPath, cudaResultsPath string) (MeasuredCostModel, error) {
	...
	thresholds, err := deriveThresholds(cpuResults, cudaResults)
	if err != nil {
		return MeasuredCostModel{}, err
	}
...
func deriveThresholds(cpuResults, cudaResults []benchfmt.Result) (map[OpKind]int, error) {
	type sample struct {
		work   int
		cpuMs  float64
		cudaMs float64
	...
	if len(grouped) == 0 {
		return nil, fmt.Errorf("no overlapping cpu/cuda benchmark results found")
	}
