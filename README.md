...
- configurable CPU/GPU execution planner with threshold, measured, and adaptive modes
- demo program
...
Or run with the online adaptive planner, which learns backend preferences from observed execution time:

```bash
go run -tags cuda ./cmd/demo -cost-model adaptive
```
...
python scripts/plot_planner_results.py --results results/planner_threshold.json results/planner_measured.json results/planner_adaptive.json --trace results/planner_adaptive_trace.json --cpu-results results/runtime_cpu.json --cuda-results results/runtime_cuda.json --output-dir results/planner_plots
```
...
It also exports planner-comparison files and adaptive traces, then generates planner plots under `results/planner_plots/`.

## Notes

- The CUDA matmul kernel is a shared-memory tiled kernel.
- It is a baseline kernel, not a cuBLAS replacement.
- The planner supports static threshold, measured threshold, and online adaptive modes.
